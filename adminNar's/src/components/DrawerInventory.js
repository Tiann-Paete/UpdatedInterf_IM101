...
const DrawerInventory = () => {
  ...
  const fetchProducts = async (page) => {
    try {
      const response = await axios.get(`http://localhost:8000/products?page=${page}&limit=10`);
  ...
  const handleUpdateProduct = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:8000/products/${editingProduct.id}`, editingProduct);
              ...
              <td className="border p-2">
                {product.image_url && (
                  <img 
                    ...
                    onClick={() => handleImageClick(product.image_url.startsWith('http') ? product.image_url : `http://localhost:8000${product.image_url}`)}
                  />
                )}
              </td>
